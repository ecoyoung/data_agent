...
- 本文档是人工可读的指标口径说明；运行时表路由、负例、跨表关系和 scope alias 以机器可读配置为准。
- 表族 AI_HINT、触发词、排除规则维护在 `src/data_agent/data_catalog/hints.json`。
- 跨表 JOIN 关系和必须预聚合的规则维护在 `src/data_agent/data_catalog/relationships.json`。
- 品牌/项目简称和 scope 解析维护在 `src/data_agent/data_catalog/scope_aliases.json`，并由 `scripts/build_scope_aliases.py` 生成或同步。
- 如果本文档和上述 JSON 配置冲突，先以 JSON 配置为准，并同步修正文档，避免 prompt 口径和 deterministic checker 分裂。
- `tables_metadata.json` 中的 `select_permission` 是生成快照；执行前仍会用当前数据库用户做 SELECT 权限预检。当前内测账号不一定能查询所有 `intermediate_amazon_` 中间表。
...
- 用户使用简称时先经过 scope alias resolver，例如 `BKN US` 应收窄到 `beekeeper_us`，裸 `BKN` 若同时命中 US、CA、整体店铺则先追问。
- 如果候选表当前账号没有 SELECT 权限，不继续生成 SQL；直接返回权限范围提示。
